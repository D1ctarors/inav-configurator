...
GUI_control.prototype.content_ready = function (callback) {
    ...
    $("<a>")
        .attr("href", "https://github.com/iNavFlight/inav/wiki")
        .attr("target", "_blank")
        .attr("id", "button-documentation")
        .html(i18n.getMessage("documentation"))
        .appendTo(documentationDiv);
    documentationDiv.insertAfter(tabTitle);

    // loading tooltip
    jQuery(document).ready(function ($) {
        $("cf_tip").each(function () {
            // Grab all ".cf_tip" elements, and for each...
...
GUI_control.prototype.updateStatusBar = function () {
    ...
    for (var i = 0; i < 32; i++) {
        var checkBit = 1 << i;
        if (
            Object.values(armingFlags).includes(checkBit) &&
            checkBit & FC.CONFIG.armingFlags
        ) {
            activeArmFlags.push(
                Object.keys(armingFlags)[
                    Object.values(armingFlags).indexOf(checkBit)
                ]
            );
        }
    }
    ...
    $("span.cpu-load").text(
        i18n.getMessage("statusbar_cpu_load", [FC.CONFIG.cpuload])
    );
    $("span.arming-flags").text(
        activeArmFlags.length ? activeArmFlags.join(", ") : "-"
    );
};
...
GUI_control.prototype.updateProfileChange = function (refresh) {
    ...
    if (refresh > 0) {
        if (refresh & GUI.PROFILES_CHANGED.CONTROL) {
            GUI.log(
                i18n.getMessage("pidTuning_LoadedProfile", [
                    FC.CONFIG.profile + 1,
                ])
            );
        }
        if (refresh & GUI.PROFILES_CHANGED.MIXER) {
            GUI.log(
                i18n.getMessage("loadedMixerProfile", [
        ...
        if (refresh & GUI.PROFILES_CHANGED.BATTERY) {
            GUI.log(
                i18n.getMessage("loadedBatteryProfile", [
...
GUI_control.prototype.fillSelect = function (
    $element,
    values,
    currentValue,
    unit
    ...
    if (
        currentValue != null &&
        $element.find('[value="' + currentValue + '"]').length == 0
    ) {
        $element.append(
            '<option value="' +
                currentValue +
...
GUI_control.prototype.simpleBind = function () {
    $("input[data-simple-bind]")
        .not('[data-simple-binded="true"]')
        .each(function () {
            var $this = $(this),
            ...
            if (
                toBind.length !== 2 ||
                window[toBind[0]][toBind[1]] === undefined
            ) {
                return;
            }
...
GUI_control.prototype.renderOperandValue = function (
    $container,
    operandMetadata,
    operand,
    value,
    ...
    switch (operandMetadata.type) {
        case "value":
            $container.append(
                '<input type="number" class="logic_element__operand--value" data-operand="' +
                    operand +
                    '" step="' +
                    operandMetadata.step +
                    '" min="' +
                    operandMetadata.min +
                    '" max="' +
                    operandMetadata.max +
        ...
        case "dictionary":
            $container.append(
                '<select class="logic_element__operand--value" data-operand="' +
                    operand +
                    '"></select>'
            );
            ...
            if (operandMetadata.type == "range") {
                for (
                    let i = operandMetadata.range[0];
                    i <= operandMetadata.range[1];
                    i++
                ) {
 ...
 */
GUI_control.prototype.renderLogicConditionSelect = function (
    $container,
    logicConditions,
    current,
    onChange,
    ...
    let $select = $container
            .append('<select class="mix-rule-condition">')
            .find("select"),
...
GUI_control.prototype.update_dataflash_global = function () {
    ...
    if (supportsDataflash) {
        ...
        $(".dataflash-free_global div").html(
            i18n.getMessage("sensorDataFlashFreeSpace") +
                formatFilesize(FC.DATAFLASH.totalSize - FC.DATAFLASH.usedSize)
        );
    } else {
        $(".noflash_global").css({
            display: "block",
