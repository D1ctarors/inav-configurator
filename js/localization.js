...
const availableLanguages = ["en", "ja", "uk", "zh_CN", "ru"];
    ...
    i18next.init(
        ...
        function (err) {
            if (err !== undefined) {
                ...
                const detectedLanguage = i18n.getMessage(
                    `language_${i18n.getValidLocale("DEFAULT")}`
                );
...
i18n.getMessage = function (messageID, parameters) {
    ...
    if (
        ...
        (parameters.constructor !== Array && parameters instanceof Object)
    ) {
...
i18n.localize = function (reTranslate = false) {
    ...
    if (reTranslate) {
        ...
        $("[i18n_placeholder]").each(function () {
            ...
            element.attr(
                "placeholder",
                translate(element.attr("i18n_placeholder"))
            );
        });
