...
TABS.gps.initialize = function (callback) {
    ...
    var loadChain = [mspHelper.loadFeatures, mspHelper.loadMiscV2];
    ...
    function process_html() {
        ...
        for (i = 0; i < gpsProtocols.length; i++) {
            gps_protocol_e.append(
                '<option value="' + i + '">' + gpsProtocols[i] + "</option>"
            );
        }
        ...
        for (i = 0; i < gpsSbas.length; i++) {
            gps_ubx_sbas_e.append(
                '<option value="' + i + '">' + gpsSbas[i] + "</option>"
            );
        }
        ...
        function update_ui() {
            ...
            $(".GPS_info td.alt").text(GPS_DATA.alt + " м");
            $(".GPS_info td.lat").text(lat.toFixed(4) + " deg");
            $(".GPS_info td.lon").text(lon.toFixed(4) + " deg");
            $(".GPS_info td.speed").text(GPS_DATA.speed + " см/с");
            $(".GPS_info td.sats").text(GPS_DATA.numSat);
            $(".GPS_info td.distToHome").text(GPS_DATA.distanceToHome + " м");
            ...
            $(".GPS_stat td.eph").text((GPS_DATA.eph / 100).toFixed(2) + " м");
            $(".GPS_stat td.epv").text((GPS_DATA.epv / 100).toFixed(2) + " м");
            ...
            if (GPS_DATA.fix >= 2) {
                ...
                if (!cursorInitialized) {
                    ...
                    iconGeometry = new ol.geom.Point(
        ...
        $("a.save").on("click", function () {
            if (FC.isFeatureEnabled("GPS", features)) {
                googleAnalytics.sendEvent(
                    "Setting",
                    "GpsProtocol",
                    gpsProtocols[MISC.gps_type]
                );
                googleAnalytics.sendEvent(
                    "Setting",
                    "GpsSbas",
                    gpsSbas[MISC.gps_ubx_sbas]
                );
            }
            ...
            googleAnalytics.sendEvent(
                "Setting",
                "GPSEnabled",
                FC.isFeatureEnabled("GPS", features) ? "true" : "false"
            );
