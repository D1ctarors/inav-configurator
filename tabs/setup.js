...
TABS.setup.initialize = function (callback) {
    ...
    function process_html() {
        ...
        if (!FC.isMotorOutputEnabled()) {
            GUI_control.prototype.log(
                "<span style='color: red; font-weight: bolder'><strong>" +
                    chrome.i18n.getMessage("logPwmOutputDisabled") +
                    "</strong></span>"
            );
        }
        ...
        $("span.roll").text(
            chrome.i18n.getMessage("initialSetupAttitude", [0])
        );
        ...
        $("div#interactive_block > a.reset").text(
            chrome.i18n.getMessage("initialSetupButtonResetZaxisValue", [
                self.yaw_fix,
            ])
        );
            ...
            if (have_sensor(CONFIG.activeSensors, "gps")) {
                ...
                MSP.send_message(
                    ...
                    function () {
                        ...
                        gpsLat_e.text(
                            (GPS_DATA.lat / 10000000).toFixed(4) + " deg"
                        );
                        gpsLon_e.text(
                            (GPS_DATA.lon / 10000000).toFixed(4) + " deg"
                        );
                    }
                );
            }
        ...
        function get_fast_data() {
            ...
            MSP.send_message(MSPCodes.MSP_ATTITUDE, false, false, function () {
                roll_e.text(
                    chrome.i18n.getMessage("initialSetupAttitude", [
        ...
        helper.mspBalancedInterval.add(
            "setup_data_pull_fast",
            40,
            1,
            get_fast_data
        );
        ...
        helper.interval.add(
            ...
            function () {
                ...
                bat_voltage_e.text(
                    chrome.i18n.getMessage("initialSetupBatteryVoltageValue", [
                        ANALOG.voltage,
                    ])
                );
                remaining_capacity_wh_decimals =
                    ANALOG.battery_remaining_capacity.toString().length < 5
                        ? 3
                        : 7 -
                          ANALOG.battery_remaining_capacity.toString().length;
                remaining_capacity_value =
                    MISC.battery_capacity_unit == "mAh"
                        ? ANALOG.battery_remaining_capacity
                        : (ANALOG.battery_remaining_capacity / 1000).toFixed(
                              remaining_capacity_wh_decimals < 0
                                  ? 0
                                  : remaining_capacity_wh_decimals
                          );
                remaining_capacity_unit =
                    MISC.battery_capacity_unit == "mAh" ? "mAh" : "Вт/ч";
                bat_remaining_e.text(
                    chrome.i18n.getMessage(
                        "initialSetupBatteryRemainingCapacityValue",
                        MISC.battery_capacity > 0 &&
                            ANALOG.battery_full_when_plugged_in
                            ? [
                                  remaining_capacity_value,
                                  remaining_capacity_unit,
                              ]
                ...
                bat_percent_e.text(
                    chrome.i18n.getMessage(
                        ...
                        [ANALOG.battery_percentage]
                    )
                );
                bat_full_e.text(
                    chrome.i18n.getMessage("initialSetupBatteryFullValue", [
                        ANALOG.battery_full_when_plugged_in,
                    ])
                );
                bat_thresh_e.text(
                    chrome.i18n.getMessage(
                        ...
                        [ANALOG.use_capacity_thresholds]
                    )
                );
                bat_mah_drawn_e.text(
                    chrome.i18n.getMessage("initialSetupBatteryMahValue", [
                        ANALOG.mAhdrawn,
                    ])
                );
                capacity_drawn_decimals =
                    ANALOG.mWhdrawn.toString().length < 5
                        ? 3
                        : 7 - ANALOG.mWhdrawn.toString().length;
                bat_wh_drawn_e.text(
                    chrome.i18n.getMessage("initialSetup_Wh_drawnValue", [
                        (ANALOG.mWhdrawn / 1000).toFixed(
                            capacity_drawn_decimals < 0
                                ? 0
                                : capacity_drawn_decimals
                        ),
                    ])
                );
                bat_current_draw_e.text(
                    chrome.i18n.getMessage("initialSetupCurrentDrawValue", [
                        ANALOG.amperage.toFixed(2),
                    ])
                );
                bat_power_draw_e.text(
                    chrome.i18n.getMessage("initialSetupPowerDrawValue", [
                        ANALOG.power.toFixed(2),
                    ])
                );
                rssi_e.text(
                    chrome.i18n.getMessage("initialSetupRSSIValue", [
                        ((ANALOG.rssi / 1023) * 100).toFixed(0),
                    ])
                );
            },
            100,
...
TABS.setup.initialize3D = function () {
    ...
    if (
        ...
        (detector_canvas.getContext("webgl") ||
            detector_canvas.getContext("experimental-webgl"))
    ) {
    ...
    if (useWebGlRenderer) {
        if (MIXER_CONFIG.appliedMixerPreset === -1) {
            ...
            GUI_control.prototype.log(
                "<span style='color: red; font-weight: bolder'><strong>" +
                    chrome.i18n.getMessage("mixerNotConfigured") +
                    "</strong></span>"
            );
        } else {
            model_file = helper.mixer.getById(
                MIXER_CONFIG.appliedMixerPreset
            ).model;
        }
    } else {
        model_file = "fallback";
    ...
    this.render3D = function () {
        ...
        // compute the changes
        model.rotation.x =
            SENSOR_DATA.kinematics[1] * -1.0 * 0.017453292519943295;
        modelWrapper.rotation.y =
            (SENSOR_DATA.kinematics[2] * -1.0 - self.yaw_fix) *
            0.017453292519943295;
        model.rotation.z =
            SENSOR_DATA.kinematics[0] * -1.0 * 0.017453292519943295;
