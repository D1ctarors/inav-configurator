...
TABS.magnetometer.initialize = function (callback) {
    ...
    var loadChain = [
        ...
        function (callback) {
            self.boardAlignmentConfig.pitch = Math.round(
                BOARD_ALIGNMENT.pitch / 10
            );
            self.boardAlignmentConfig.roll = Math.round(
                BOARD_ALIGNMENT.roll / 10
            );
            self.boardAlignmentConfig.yaw = Math.round(
                BOARD_ALIGNMENT.yaw / 10
            );
            callback();
        },
    ...
    function areAnglesZero() {
        return (
            self.alignmentConfig.pitch === 0 &&
            self.alignmentConfig.roll === 0 &&
            self.alignmentConfig.yaw === 0
        );
    }

    function isBoardAlignmentZero() {
        return (
            self.boardAlignmentConfig.pitch == 0 &&
    ...
    var saveChain = [
        ...
        function (callback) {
            if (self.isSavePreset)
                ...
                mspHelper.setSetting(
                    "align_mag_roll",
                    self.alignmentConfig.roll * 10,
                    callback
                );
        },
        function (callback) {
        ...
        function (callback) {
            if (self.isSavePreset)
                ...
                mspHelper.setSetting(
                    "align_mag_yaw",
                    self.alignmentConfig.yaw * 10 + fix,
                    callback
                );
            }
        },
    ...
    function getAxisDegreeWithPresetAndBoardOrientation(selectedPreset) {
        ...
        var magRotation = new THREE.Euler(
            -THREE.Math.degToRad(degree[0] - 180),
            THREE.Math.degToRad(-180 - degree[2]),
            THREE.Math.degToRad(degree[1]),
            "YXZ"
        );
        var matrix = new THREE.Matrix4().makeRotationFromEuler(magRotation);

        var boardRotation = new THREE.Euler(
        ...
        var pitch = toUpperRange(
            Math.round(THREE.Math.radToDeg(-euler.x)) + 180,
            180
        );
        var yaw = toUpperRange(
            Math.round(-180 - THREE.Math.radToDeg(euler.y)),
            359
        );
    ...
    function updateMagOrientationWithPreset() {
        if (self.isSavePreset) {
            const degrees = getAxisDegreeWithPresetAndBoardOrientation(
                SENSOR_ALIGNMENT.align_mag
            );
    ...
    function updateBoardRollAxis(value) {
        ...
        self.pageElements.orientation_board_roll.val(
    ...
    function updateBoardPitchAxis(value) {
        ...
        self.pageElements.board_pitch_slider.val(
            self.boardAlignmentConfig.pitch
        );
        self.pageElements.orientation_board_pitch.val(
            self.boardAlignmentConfig.pitch
        );
        updateMagOrientationWithPreset();
        self.render3D();
    ...
    function updateBoardYawAxis(value) {
        ...
        self.pageElements.orientation_board_yaw.val(
    ...
    function disableSavePreset() {
        ...
        self.pageElements.orientation_mag_e.css(
            "text-decoration",
            "line-through"
        );
    }
    ...
    function process_html() {
        ...
        for (i = 0; i < alignments.length; i++) {
            self.pageElements.orientation_mag_e.append(
                '<option value="' + (i + 1) + '">' + alignments[i] + "</option>"
            );
        }
...
TABS.magnetometer.initialize3D = function () {
    ...
    this.render3D = function () {
        ...
        var magRotation = new THREE.Euler(
            -THREE.Math.degToRad(self.alignmentConfig.pitch - 180),
            THREE.Math.degToRad(-180 - self.alignmentConfig.yaw),
            THREE.Math.degToRad(self.alignmentConfig.roll),
            "YXZ"
        );
        var matrix = new THREE.Matrix4().makeRotationFromEuler(magRotation);
...
*/
        ...
        // draw
        if (camera != null) renderer.render(scene, camera);
    };
