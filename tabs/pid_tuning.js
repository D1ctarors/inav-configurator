...
TABS.pid_tuning.initialize = function (callback) {
    ...
    function form_to_pid_and_rc() {
        ...
        RATE_DYNAMICS.sensitivityCenter = parseInt(
            $("#rate_dynamics_center_sensitivity").val()
        );
        RATE_DYNAMICS.sensitivityEnd = parseInt(
            $("#rate_dynamics_end_sensitivity").val()
        );
        RATE_DYNAMICS.correctionCenter = parseInt(
            $("#rate_dynamics_center_correction").val()
        );
        RATE_DYNAMICS.correctionEnd = parseInt(
            $("#rate_dynamics_end_correction").val()
        );
        RATE_DYNAMICS.weightCenter = parseInt(
            $("#rate_dynamics_center_weight").val()
        );
        RATE_DYNAMICS.weightEnd = parseInt(
            $("#rate_dynamics_end_weight").val()
        );
    }
    ...
    function process_html() {
            ...
            if (confirm(chrome.i18n.getMessage("confirm_select_defaults"))) {
                mspHelper.setSetting("applied_defaults", 0, function () {
                    mspHelper.saveToEeprom(function () {
                        GUI.log(
                            chrome.i18n.getMessage("configurationEepromSaved")
                        );
        ...
        $(".pid-slider-row [name='value-slider']").on("input", function () {
            ...
            PIDs[$(this).parent().data("axis")][$(this).parent().data("bank")] =
                val;
        });
        ...
        $(".pid-slider-row [name='value-input']").on("change", function () {
            let val = $(this).val();
            let newVal;
            let normalMax = parseInt(
                $(this)
                    .parent()
                    .find('input[name="value-slider"]')
                    .data("normal-max")
            );
        ...
        GUI.sliderize(
            ...
            RATE_DYNAMICS.sensitivityCenter,
            25,
            175
        );
        GUI.sliderize(
            $("#rate_dynamics_end_sensitivity"),
            RATE_DYNAMICS.sensitivityEnd,
            25,
            175
        );
        ...
        GUI.sliderize(
            ...
            RATE_DYNAMICS.correctionCenter,
            10,
            95
        );
        GUI.sliderize(
            $("#rate_dynamics_end_correction"),
            RATE_DYNAMICS.correctionEnd,
            10,
            95
        );
        ...
        GUI.sliderize(
            ...
            RATE_DYNAMICS.weightCenter,
            0,
            95
        );
        GUI.sliderize(
            $("#rate_dynamics_end_weight"),
            RATE_DYNAMICS.weightEnd,
            0,
            95
        );
        ...
        $("a.update").click(function () {
            ...
            function saveINAVPidConfig() {
                MSP.send_message(
                    MSPCodes.MSP_SET_INAV_PID,
                    mspHelper.crunch(MSPCodes.MSP_SET_INAV_PID),
                    false,
                    savePidAdvanced
                );
            }
