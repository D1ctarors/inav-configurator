...
TABS.receiver.initialize = function (callback) {
    ...
    function drawRollPitchExpo() {
        ...
        if (
            expoA <= parseFloat(expoAVal.prop("min")) ||
            expoA >= parseFloat(expoAVal.prop("max")) ||
            expoM <= parseFloat(expoMVal.prop("min")) ||
            expoM >= parseFloat(expoMVal.prop("max"))
        ) {
            return;
        }
        ...
        context.quadraticCurveTo(
            110,
            rateHeight - (rateHeight / 2) * (1 - expoA),
            200,
            0
        );
        context.lineWidth = 2;
        ...
        context.quadraticCurveTo(
            110,
            rateHeight - (rateHeight / 2) * (1 - expoM),
            200,
            0
        );
        context.lineWidth = 2;
    ...
    function process_html() {
        ...
        $('.tunings .throttle input[name="mid"]').val(
            RC_tuning.throttle_MID.toFixed(2)
        );
        $('.tunings .throttle input[name="expo"]').val(
            RC_tuning.throttle_EXPO.toFixed(2)
        );

        $('.tunings .rate input[name="expo"]').val(
            RC_tuning.RC_EXPO.toFixed(2)
        );
        $('.tunings .yaw_rate input[name="yaw_expo"]').val(
            RC_tuning.RC_YAW_EXPO.toFixed(2)
        );

        $('.tunings .rate input[name="manual_expo"]').val(
            RC_tuning.manual_RC_EXPO.toFixed(2)
        );
        $('.tunings .yaw_rate input[name="manual_yaw_expo"]').val(
            RC_tuning.manual_RC_YAW_EXPO.toFixed(2)
        );
        ...
        $rcMap.focusout(function () {
            if (!FC.isRcMapValid($(this).val())) $(this).val(last_valid);
        });

        $rcMap.on("input change", function () {
            $(this).css(
                "color",
                FC.isRcMapValid($(this).val()) ? "" : "#FF0000"
            );
        });
        ...
        // UI Hooks
        // curves
        $(".tunings .throttle input")
            .on("input change", function () {
                setTimeout(function () {
                    // let global validation trigger and adjust the values first
                    var throttleMidE = $(
                            '.tunings .throttle input[name="mid"]'
                        ),
                        throttleExpoE = $(
                            '.tunings .throttle input[name="expo"]'
                        ),
                    ...
                    var midx = 200 * mid,
                        ...
                        midyl =
                            rateHeight - (rateHeight - midy) * 0.5 * (expo + 1),
                        midyr = (midy / 2) * (expo + 1);
        ...
        $("a.update").click(function () {
            ...
            RC_tuning.RC_EXPO = parseFloat(
                $('.tunings .rate input[name="expo"]').val()
            );
            RC_tuning.RC_YAW_EXPO = parseFloat(
                $('.tunings .yaw_rate input[name="yaw_expo"]').val()
            );

            RC_tuning.manual_RC_EXPO = parseFloat(
                $('.tunings .rate input[name="manual_expo"]').val()
            );
            RC_tuning.manual_RC_YAW_EXPO = parseFloat(
                $('.tunings .yaw_rate input[name="manual_yaw_expo"]').val()
            );

            RC_deadband.yaw_deadband = parseInt(
                $('.deadband input[name="yaw_deadband"]').val()
            );
            RC_deadband.deadband = parseInt(
                $('.deadband input[name="deadband"]').val()
            );
            ...
            googleAnalytics.sendEvent(
                "Setting",
                "RcProtocol",
                $("#receiver_type option:selected").text() +
                    ":" +
                    $("#serialrx_provider option:selected").text()
            );

            // catch rssi aux
            MISC.rssi_channel = parseInt(
                $('select[name="rssi_channel"]').val()
            );
            ...
            function save_misc() {
                MSP.send_message(
                    ...
                    mspHelper.crunch(MSPCodes.MSP_SET_MISC),
                    false,
                    save_rc_configs
                );
            }
            ...
            MSP.send_message(
                MSPCodes.MSPV2_INAV_SET_RATE_PROFILE,
                mspHelper.crunch(MSPCodes.MSPV2_INAV_SET_RATE_PROFILE),
                false,
                save_rc_map
            );
        });
        ...
        mspHelper.getSetting("receiver_type").then(function (s) {
            if (s && s.setting.table && s.setting.table.values) {
                $(".sticks_btn").toggle(
                    s.setting.table.values[s.value] == "MSP"
                );
            }
        });
        ...
        function update_ui() {
            ...
            for (i = 0; i < RC.active_channels; i++) {
                meter_fill_array[i].css(
                    "width",
                    (
                        ((RC.channels[i] - meter_scale.min) /
                            (meter_scale.max - meter_scale.min)) *
                        100
                    ).clamp(0, 100) + "%"
                );
