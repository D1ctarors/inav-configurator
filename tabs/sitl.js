    ...
    GUI.load("./tabs/sitl.html", function () {
        ...
        if (os != "Windows" && os != "Linux") {
            ...
            $(".content_wrapper").append(
                `<h2>${chrome.i18n.getMessage("sitlOSNotSupported")}</h2>`
            );
        ...
        $("#sitlLog").animate(
            { scrollTop: $("#sitlLog").scrollHeight },
            "fast"
        );

        profiles = stdProfiles.slice(0);
        chrome.storage.local.get("sitlProfiles", (result) => {
            if (result.sitlProfiles) profiles.push(...result.sitlProfiles);
        ...
        port_e.on("change", () => {
            if (!currentSim.isPortFixed) {
                ...
                if (port != NaN) currentProfile.port = parseInt(port_e.val());
            }
        });
        ...
        $(".sitlStart").on("click", () => {
            ...
            for (let i = 0; i < currentSim.inputChannels; i++) {
                ...
                if (inavChan == 0) {
                    ...
                    channelMap += `M${zeroPad(inavChan, 2)}-${zeroPad(
                        i + 1,
                        2
                    )},`;
                } else {
                    channelMap += `S${zeroPad(inavChan - 12, 2)}-${zeroPad(
                        i + 1,
                        2
                    )},`;
                }
            ...
            SITLProcess.start(
                ...
                (result) => {
                    appendLog(result);
                    if (
                        serialTcpEnable.is(":checked") &&
                        result ==
                            `[SIM] Connection with ${currentProfile.sim} successfully established.\n`
                    ) {
                        Ser2TCP.start(
                            serialPorts_e.val(),
                            serialOptions,
                            currentProfile.ip,
                            currentProfile.tcpPort,
                            (result) => {
        ...
        profileNewBtn_e.on("click", () => {
            var name = prompt(
                chrome.i18n.getMessage("sitlNewProfile"),
                chrome.i18n.getMessage("sitlEnterName")
            );
            if (!name) return;
            ...
            var eerpromName =
                name.replace(/[^a-z0-9]/gi, "_").toLowerCase() + ".bin";
            var profile = {
                name: name,
                sim: "RealFlight",
                isStdProfile: false,
        ...
        baudRate_e.on("change", () => {
            ...
            if (baud != NaN) currentProfile.baudrate = baud;
        });
        ...
        function initElements(init) {
            ...
            if (init) {
                ...
                serialProtocolls.forEach((protocoll) => {
                    protocollPreset_e.append(
                        `<option value="${protocoll.name}">${protocoll.name}</option>`
                    );
                });
        ...
        function saveProfiles() {
            ...
            var profilesToSave = [];
            profiles.forEach((profile) => {
                if (!profile.isStdProfile) profilesToSave.push(profile);
            });
        ...
        function updateCurrentProfile() {
            ...
            enableSim_e
                .prop("checked", currentProfile.simEnabeld)
                .trigger("change");
            serialTcpEable_e
                .prop("checked", currentProfile.useSerialTcp)
                .trigger("change");
        ...
        function renderChanMapTable() {
            ...
            for (let i = 0; i < currentSim.inputChannels; i++) {
                ...
                mapTableBody.append(
                    "<tr><td>" +
                        output +
                        '</td><td><select data-out="' +
                        i +
                        '" class="inavChannel""></select></td></td>'
                );
                const row = mapTableBody.find("tr:last");
                GUI.fillSelect(
                    row.find(".inavChannel"),
                    getInavChannels(),
                    mapping[i]
                );
                ...
                row.find(".inavChannel")
                    .val(mapping[i])
                    .on("change", (sender) => {
                        mapping[$(sender.target).data("out")] = parseInt(
                            $(sender.target).val()
                        );
        ...
        function getInavChannels() {
            var channels = [];
            for (var i = 0; i <= 12; i++) {
                if (i == 0) channels.push("None");
                else channels.push("Motor " + i);
            }

            for (var i = 1; i <= 16; i++) {
                if (i == 0) channels.push("None");
                else channels.push("Servo " + i);
            }
            return channels;
        }
        ...
        function appendLog(message) {
            ...
            $("#sitlLog").val(SITL_LOG);
            $("#sitlLog").animate(
                { scrollTop: $("#sitlLog")[0].scrollHeight },
                "fast"
            );
        }
