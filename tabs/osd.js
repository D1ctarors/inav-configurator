...
SYM.AH_CROSSHAIRS = new Array(
    0x166,
    0x1a4,
    new Array(0x190, 0x191, 0x192),
    new Array(0x193, 0x194, 0x195),
    new Array(0x196, 0x197, 0x198),
    new Array(0x199, 0x19a, 0x19b),
    new Array(0x19c, 0x19d, 0x19e),
    new Array(0x19f, 0x1a0, 0x1a1)
);
...
FONT.parseMCMFontFile = function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (
            character_bits.length ==
            FONT.constants.SIZES.MAX_NVM_FONT_CHAR_FIELD_SIZE * (8 / 2)
        ) {
            pushChar();
        }
...
FONT.draw = function (charAddress) {
    ...
    if (!cached) {
        cached = FONT.data.character_image_urls[charAddress] =
            drawCanvas(charAddress).toDataURL("image/png");
    }
    return cached;
};
...
FONT.msp = {
    encode: function (charAddress) {
        ...
        return addr.concat(
            data.slice(0, FONT.constants.SIZES.MAX_NVM_FONT_CHAR_SIZE)
        );
    },
};
    ...
    addrs
        .reduce(function (p, next, idx) {
            return p.then(function () {
                ...
                var proto =
                    next <= 255
                        ? MSP.constants.PROTOCOL_V1
                        : MSP.constants.PROTOCOL_V2;
...
function osdCoordinatePreview(symbol, coordinate) {
    return function () {
        ...
        return (
            FONT.symbol(symbol) +
            FONT.embed_dot(coordinate.toFixed(digits - integerLength))
        );
    };
}
...
OSD.DjiElements = {
    ...
    supportedSettings: ["craft_name", "units"],
    supportedAlarms: ["rssi_alarm", "osd_alt_alarm"],
    craftNameElements: [
        "MESSAGES",
        "THROTTLE_POSITION",
...
OSD.constants = {
    ...
    ALL_ALARMS: [
        ...
        },
        {
            ...
            to_display: altitude_alarm_display_function(
                altitude_alarm_to_display
            ),
            from_display: altitude_alarm_display_function(
                altitude_alarm_from_display
            ),
            min: 0,
    ...
    ALL_DISPLAY_GROUPS: [
        {
            ...
            items: [
                ...
                },
                {
                    ...
                    id: 32,
                    preview:
                        FONT.symbol(SYM.BATT) +
                        FONT.embed_dot("3.90") +
                        FONT.symbol(SYM.VOLT),
                },
                {
                ...
                },
                {
                    ...
                    preview: function (osd_data) {
                        switch (OSD.data.preferences.units) {
                            ...
                            case 3: // UK
                                return (
                                    FONT.symbol(SYM.FLIGHT_DIST_REMAINING) +
                                    FONT.embed_dot("0.98") +
                                    FONT.symbol(SYM.DIST_MI)
                                );
                            case 4: // GA
                                return (
                                    FONT.symbol(SYM.FLIGHT_DIST_REMAINING) +
                                    FONT.embed_dot("0.85") +
                                    FONT.symbol(SYM.DIST_NM)
                                );
                            default: // Metric
                                return (
                                    FONT.symbol(SYM.FLIGHT_DIST_REMAINING) +
                                    FONT.embed_dot("1.73") +
                                    FONT.symbol(SYM.DIST_KM)
                                );
                        }
                    },
                },
                ...
                },
                {
                    ...
                    id: 33,
                    preview:
                        FONT.symbol(SYM.SCALE) + FONT.symbol(SYM.THR) + " 51",
                },
                {
                    name: "CRAFT_NAME",
                    id: 8,
                ...
                },
                {
                    ...
                    id: 146,
                    preview:
                        FONT.symbol(SYM.PILOT_LOGO_SML_L) +
                        FONT.symbol(SYM.PILOT_LOGO_SML_C) +
                        FONT.symbol(SYM.PILOT_LOGO_SML_R),
                },
                {
                    name: "FLYMODE",
                    id: 7,
                ...
                },
                {
                    ...
                    id: 24,
                    preview:
                        FONT.symbol(SYM.HEADING) +
                        "175" +
                        FONT.symbol(SYM.DEGREES),
                },
                {
                    name: "HEADING_GRAPH",
                    id: 34,
                ...
                },
                {
                    ...
                    min_version: "3.0.0",
                    preview:
                        FONT.symbol(SYM.GLIDESLOPE) + FONT.embed_dot("12.3"),
                },
                {
                    name: "GLIDE_TIME",
                    id: 136,
                ...
                },
                {
                    ...
                    min_version: "6.0.0",
                    preview:
                        FONT.symbol(SYM.PAN_SERVO_IS_OFFSET_L) +
                        "120" +
                        FONT.symbol(SYM.DEGREES),
                },
                {
            ...
            items: [
                {
                    ...
                    preview: function (osd_data) {
                        switch (OSD.data.preferences.units) {
                            case 0: // Imperial
                                return (
                                    FONT.symbol(SYM.IMU_TEMP) +
                                    " 90" +
                                    FONT.symbol(SYM.TEMP_F)
                                );
                            default: // Metric
                                return (
                                    FONT.symbol(SYM.IMU_TEMP) +
                                    " 32" +
                                    FONT.symbol(SYM.TEMP_C)
                                );
                        }
                    },
                },
                ...
                },
                {
                    ...
                    preview: function (osd_data) {
                        switch (OSD.data.preferences.units) {
                            case 0: // Imperial
                                return (
                                    FONT.symbol(SYM.ESC_TEMPERATURE) +
                                    " 98" +
                                    FONT.symbol(SYM.TEMP_F)
                                );
                            default: // Metric
                    ...
                    preview: function (osd_data) {
                        switch (OSD.data.preferences.units) {
                            ...
                            case 3: // UK
                                return (
                                    FONT.embed_dot("-1.6") +
                                    FONT.symbol(SYM.FT_S)
                                );
                            case 4: // GA
                                return (
                                    FONT.embed_dot("-2.6") +
                                    FONT.symbol(SYM.HUND_FTM)
                                );
                            default: // Metric
                                return (
                                    FONT.embed_dot("-0.5") +
        ...
        },
        {
            ...
            items: [
                ...
                },
                {
                    ...
                    min_version: "2.2.0",
                    preview:
                        FONT.symbol(SYM.GFORCE_X) + FONT.embed_dot("-0.10"),
                },
                {
                    name: "G_FORCE_Y",
                    id: 102,
                    min_version: "2.2.0",
                    preview:
                        FONT.symbol(SYM.GFORCE_Y) + FONT.embed_dot("-0.20"),
                },
                {
                    name: "G_FORCE_Z",
                    id: 103,
                    min_version: "2.2.0",
                    preview:
                        FONT.symbol(SYM.GFORCE_Z) + FONT.embed_dot("-0.30"),
                },
            ],
        },
        {
        ...
        },
        {
            ...
            items: [
                ...
                },
                {
                    ...
                    preview: function () {
                        return (
                            FONT.symbol(SYM.PITCH_UP) + FONT.embed_dot(" 1.5")
                        );
                    },
                },
                {
                    ...
                    preview: function () {
                        return (
                            FONT.symbol(SYM.ROLL_LEFT) + FONT.embed_dot("31.4")
                        );
                    },
                },
            ],
                    ...
                    preview: function (osd_data) {
                        switch (OSD.data.preferences.units) {
                            ...
                            case 3: // UK
                                return (
                                    "116" +
                                    FONT.symbol(SYM.MAH_MI_0) +
                                    FONT.symbol(SYM.MAH_MI_1)
                                );
                            case 4: // GA
                                return (
                                    "101" +
                                    FONT.symbol(SYM.MAH_NM_0) +
                                    FONT.symbol(SYM.MAH_NM_1)
                                );
                            default: // Metric
                                return (
                                    "187" +
                                    FONT.symbol(SYM.MAH_KM_0) +
                                    FONT.symbol(SYM.MAH_KM_1)
                                );
                        }
                    },
                },
                    ...
                    preview: function (osd_data) {
                        switch (OSD.data.preferences.units) {
                            ...
                            case 3: // UK
                                return (
                                    FONT.embed_dot("0.76") +
                                    FONT.symbol(SYM.WH_MI)
                                );
                            case 4: // GA
                                return (
                                    FONT.embed_dot("0.66") +
                                    FONT.symbol(SYM.WH_NM)
                                );
                            default: // Metric
                                return (
                                    FONT.embed_dot("1.23") +
                                    FONT.symbol(SYM.WH_KM)
                                );
                        }
                    },
                },
                    ...
                    preview: function (osd_data) {
                        switch (OSD.data.preferences.units) {
                            ...
                            case 4: // GA
                                return (
                                    FONT.embed_dot("0.76") +
                                    FONT.symbol(SYM.AH_V_FT_0) +
                                    FONT.symbol(SYM.AH_V_FT_1)
                                );
                            default: // Metric & Metric + MPH
                                return (
                                    FONT.embed_dot("1.23") +
                                    FONT.symbol(SYM.AH_V_M_0) +
                                    FONT.symbol(SYM.AH_V_M_1)
                                );
                        }
                    },
                },
        ...
        },
        {
            ...
            items: [
                ...
                },
                {
                    ...
                    preview: function (osd_data) {
                        ...
                        switch (OSD.data.preferences.units) {
                            ...
                            case 3: // UK
                                return (
                                    FONT.embed_dot("115") + FONT.symbol(SYM.MPH)
                                );
                            case 4: // GA
                                return (
                                    FONT.embed_dot("100") + FONT.symbol(SYM.KT)
                                );
                            default: // Metric
                                return (
                                    FONT.embed_dot("185") + FONT.symbol(SYM.KMH)
                                );
                        }
                    },
                },
                    ...
                    preview: function (osd_data) {
                        ...
                        switch (OSD.data.preferences.units) {
                            ...
                            case 3: // UK
                                return (
                                    FONT.embed_dot("127") +
                                    FONT.symbol(SYM.MPH_3D)
                                );
                            case 4: // GA
                                return (
                                    FONT.embed_dot("110") +
                                    FONT.symbol(SYM.KT_3D)
                                );
                            default: // Metric
                                return (
                                    FONT.embed_dot("204") +
                                    FONT.symbol(SYM.KMH_3D)
                                );
                        }
                    },
                },
                ...
                },
                {
                    ...
                    id: 14,
                    preview:
                        FONT.symbol(SYM.GPS_SAT1) +
                        FONT.symbol(SYM.GPS_SAT2) +
                        "14",
                },
                {
                    name: "LONGITUDE",
                ...
                },
                {
                    ...
                    preview: function () {
                        let digits =
                            parseInt(
                                Settings.getInputValue("osd_plus_code_digits")
                            ) + 1;
                        let digitsRemoved =
                            parseInt(
                                Settings.getInputValue("osd_plus_code_short")
                            ) * 2;
                        console.log("DITIS", digits);
                        return "9547X6PM+VWCCC".substr(
                            digitsRemoved,
                            digits - digitsRemoved
                        );
                    },
                },
                {
                ...
                },
                {
                    ...
                    preview: function (osd_data) {
                        switch (OSD.data.preferences.units) {
                            ...
                            case 3: // UK
                                return (
                                    FONT.symbol(SYM.ODOMETER) +
                                    FONT.embed_dot("00016.9") +
                                    FONT.symbol(SYM.DIST_MI)
                                );
                            case 4: // GA
                                return (
                                    FONT.symbol(SYM.ODOMETER) +
                                    FONT.embed_dot("00014.7") +
                                    FONT.symbol(SYM.DIST_NM)
                                );
                            default: // Metric
                                return (
                                    FONT.symbol(SYM.ODOMETER) +
                                    FONT.embed_dot("00027.2") +
                                    FONT.symbol(SYM.DIST_KM)
                                );
                        }
                    },
                },
                {
                    ...
                    id: 31,
                    preview:
                        FONT.symbol(SYM.GPS_HDP1) +
                        FONT.symbol(SYM.GPS_HDP2) +
                        FONT.embed_dot("1.8"),
                },
                {
                    ...
                    preview: function (osd_data) {
                        // 6 chars
                        var p =
                            FONT.symbol(SYM.WIND_SPEED_HORIZONTAL) +
                            FONT.symbol(SYM.DIRECTION + 1);
                        switch (OSD.data.preferences.units) {
                            case 0: // Imperial
                            case 2: // Metric + MPH
                            case 3: // UK
                ...
                },
                {
                    ...
                    preview: function (osd_data) {
                        // 6 chars
                        var p =
                            FONT.symbol(SYM.WIND_SPEED_VERTICAL) +
                            FONT.symbol(SYM.AH_DECORATION_UP);
                        switch (OSD.data.preferences.units) {
                            case 0: // Imperial
                            case 2: // Metric + MPH
                            case 3: // UK
                ...
                },
                {
                    ...
                    preview: function (osd_data) {
                        switch (OSD.data.preferences.units) {
                            ...
                            case 3: // UK
                                return (
                                    FONT.symbol(SYM.CROSS_TRACK_ERROR) +
                                    FONT.embed_dot("0.98") +
                                    FONT.symbol(SYM.DIST_MI)
                                );
                            case 4: // GA
            ...
            items: [
                {
                    ...
                    positionable: true,
                    preview:
                        FONT.symbol(SYM.RSSI) + "-100" + FONT.symbol(SYM.DBM),
                },
                {
                    name: "CRSF_LQ",
                    id: 110,
                    ...
                    preview: function (osd_data) {
                        ...
                        if (Settings.getInputValue("osd_crsf_lq_format") == 0) {
                            ...
                            Settings.getInputValue("osd_crsf_lq_format") == 1
                        ) {
...
OSD.is_item_displayed = function (item, group) {
    ...
    if (
        item.min_version &&
        !semver.gte(CONFIG.flightControllerVersion, item.min_version)
    ) {
        return false;
    }
...
OSD.reload = function (callback) {
    ...
    MSP.promise(MSPCodes.MSP2_INAV_OSD_LAYOUTS).then(function (resp) {
        ...
        var ids = Array.apply(null, { length: OSD.data.layout_count }).map(
            Number.call,
            Number
        );
        var layouts = Promise.mapSeries(ids, function (layoutIndex, ii) {
            ...
            return MSP.promise(MSPCodes.MSP2_INAV_OSD_LAYOUTS, data).then(
                function (resp) {
        ...
        layouts.then(function () {
            ...
            MSP.promise(MSPCodes.MSP2_INAV_OSD_ALARMS).then(function (resp) {
                ...
                MSP.promise(MSPCodes.MSP2_INAV_OSD_PREFERENCES).then(function (
                    resp
                ) {
...
OSD.updateDisplaySize = function () {
    ...
    for (var ii = 0; ii < OSD.data.layout_count; ii++) {
        ...
        for (var jj = 0; jj < OSD.data.item_count; jj++) {
            ...
            if (
                item.x < OSD.data.display_size.x &&
                item.y < OSD.data.display_size.y
            ) {
                items[jj] = item;
            }
        }
    ...
    $(".preview").toggleClass(
        "preview_avatar cut43_left",
        video_type == "AVATAR"
    );
    $(".third_right").toggleClass(
        "preview_avatar_side",
        video_type == "AVATAR"
    );
    // -- DJI WTF
    ...
    // -- BFHDCOMPAT
    $(".third_left").toggleClass(
        "preview_bfhdcompat_side",
        video_type == "BFHDCOMPAT"
    );
    $(".preview").toggleClass(
        "preview_bfhdcompat cut43_left",
        video_type == "BFHDCOMPAT"
    );
    $(".third_right").toggleClass(
        "preview_bfhdcompat_side",
...
OSD.saveConfig = function (callback) {
    return OSD.saveAlarms(function () {
        ...
        return MSP.promise(MSPCodes.MSP2_INAV_OSD_SET_PREFERENCES, data).then(
            callback
        );
    });
};
    ...
    helpers: {
        unpack: {
            position: function (bits) {
                ...
                display_item.position =
                    display_item.y * FONT.constants.SIZES.LINE + display_item.x;
        ...
        calculate: {
            coords: function (display_item) {
                display_item.x =
                    display_item.position % FONT.constants.SIZES.LINE & 0x3f;
                display_item.y =
                    (display_item.position / FONT.constants.SIZES.LINE) & 0x3f;
                return display_item;
            },
        ...
        pack: {
            position: function (display_item) {
                return (
                    (display_item.isVisible ? OSD.constants.VISIBLE : 0) |
                    ((display_item.y & 0x3f) << 6) |
                    (display_item.x & 0x3f)
                );
            },
        },
...
OSD.GUI.preview = {
    ...
    onDrop: function (e) {
        ...
        item_width = Math.max(item_width, line_width);
        var overflows_line =
            FONT.constants.SIZES.LINE -
            ((position % FONT.constants.SIZES.LINE) + item_width);
        if (overflows_line < 0) {
            position += overflows_line;
        }

        $("input." + item_id + ".position")
            .val(position)
            .change();
    },
};
...
OSD.GUI.checkAndProcessSymbolPosition = function (pos, charCode) {
    if (
        typeof OSD.data.preview[pos] === "object" &&
        OSD.data.preview[pos][0] !== null
    ) {
...
OSD.GUI.updateVideoMode = function () {
    ...
    if (OSD.data.isMspDisplay) {
        if (
            mspVideoSystem.includes(OSD.data.preferences.video_system) == false
        ) {
            OSD.data.preferences.video_system =
                OSD.constants.VIDEO_TYPES.indexOf("HDZERO");
            OSD.updateDisplaySize();
            OSD.GUI.saveConfig();
        }
    } else {
        if (
            analogVideoSystem.includes(OSD.data.preferences.video_system) ==
            false
        ) {
            OSD.data.preferences.video_system =
                OSD.constants.VIDEO_TYPES.indexOf("AUTO");
            OSD.updateDisplaySize();
            OSD.GUI.saveConfig();
        }
    }
            ...
            if (mspVideoSystem.includes(i)) {
                $videoTypes.append(
                    $(
                        ...
                        .prop(
                            "selected",
                            i === OSD.data.preferences.video_system
                        )
                        .data("type", i)
                );
            }
    ...
    $videoTypes.change(function () {
        OSD.data.preferences.video_system = $(this)
            .find(":selected")
            .data("type");
        OSD.updateDisplaySize();
        OSD.GUI.saveConfig();
...
OSD.GUI.updateUnits = function () {
    ...
    for (var i = 0; i < OSD.constants.UNIT_TYPES.length; i++) {
        ...
        if (
            unitType.min_version &&
            semver.lt(CONFIG.flightControllerVersion, unitType.min_version)
        ) {
            continue;
        }
...
OSD.GUI.updateFields = function () {
    ...
    for (var ii = 0; ii < OSD.constants.ALL_DISPLAY_GROUPS.length; ii++) {
        ...
        for (var jj = 0; jj < groupItems.length; jj++) {
            ...
            var $field = $(
                '<div class="display-field field-' + item.id + '"/>'
            );
            var name = item.name;
                ...
                $(
                    ...
                    .change(function () {
                        ...
                        if (itemData.isVisible) {
                            ...
                            if (
                                itemData.x > OSD.data.display_size.x ||
                                itemData.y > OSD.data.display_size.y
                            ) {
...
OSD.GUI.updateGuidesView = function (on) {
    isHdZero =
        OSD.constants.VIDEO_TYPES[OSD.data.preferences.video_system] ==
        "HDZERO";
    ...
    $(".hd_avatar_storage_box_top").toggleClass(
        "hd_avatar_storagebox_t",
        isAvatar && on
    );
    $(".hd_avatar_storage_box_bottom").toggleClass(
        "hd_avatar_storagebox_b",
        isAvatar && on
    );
    $(".hd_avatar_storage_box_left").toggleClass(
        "hd_avatar_storagebox_l",
        isAvatar && on
    );
    $(".hd_avatar_storage_box_right").toggleClass(
        "hd_avatar_storagebox_r",
        isAvatar && on
    );
    ...
    isBfHdCompat =
        OSD.constants.VIDEO_TYPES[OSD.data.preferences.video_system] ==
        "BFHDCOMPAT";
    $(".hd_43_margin_left").toggleClass(
        "hd_bfhdcompat_43_left",
        isBfHdCompat && on
    );
    $(".hd_43_margin_right").toggleClass(
        "hd_bfhdcompat_43_right",
        isBfHdCompat && on
    );
    $(".hd_bfhdcompat_bottom_box").toggleClass(
        "hd_bfhdcompat_bottom",
        isBfHdCompat && on
    );
    $(".hd_bfhdcompat_storage_box").toggleClass(
        "hd_bfhdcompat_storagebox",
        isBfHdCompat && on
    );

    isPAL =
        OSD.constants.VIDEO_TYPES[OSD.data.preferences.video_system] == "PAL" ||
        OSD.constants.VIDEO_TYPES[OSD.data.preferences.video_system] == "AUTO";
...
OSD.GUI.updateDjiView = function (on) {
    if (on) {
        ...
        $(".display-field").show().removeClass("no-bottom");

        $(".settings-container, .alarms-container")
            .find(".settings")
            .children()
            .show()
    ...
    OSD.GUI.updateDjiMessageElements(
        $("#useCraftnameForMessages").is(":checked")
    );
};
...
OSD.GUI.updateMapPreview = function (
    mapCenter,
    name,
    directionSymbol,
    centerSymbol
...
OSD.GUI.updatePreviews = function () {
    ...
    for (var ii = 0; ii < OSD.data.items.length; ii++) {
        ...
        if (
            OSD.DjiElements.craftNameElements.includes(item.name) &&
            $("#djiUnsupportedElements").find("input").is(":checked")
        ) {
            continue;
        }
        var j =
            itemData.position >= 0
                ? itemData.position
                : itemData.position + OSD.data.display_size.total;
    ...
    var centerPosition =
        (OSD.data.display_size.x * OSD.data.display_size.y) / 2;
    if (OSD.data.display_size.y % 2 == 0) {
        centerPosition += Math.floor(OSD.data.display_size.x / 2);
    }

    let hudCenterPosition =
        centerPosition -
        OSD.constants.VIDEO_COLS[video_type] * $("#osd_horizon_offset").val();

    // artificial horizon
    if ($('input[name="ARTIFICIAL_HORIZON"]').prop("checked")) {
        for (i = 0; i < 9; i++) {
            OSD.GUI.checkAndProcessSymbolPosition(
                hudCenterPosition - 4 + i,
                SYM.AH_BAR9_0 + 4
            );
        }
    }
    ...
    if ($('input[name="CROSSHAIRS"]').prop("checked")) {
        ...
        if (crsHNumber == 1) {
            ...
            OSD.GUI.checkAndProcessSymbolPosition(
                hudCenterPosition - 1,
                SYM.AH_CROSSHAIRS[crsHNumber][0]
            );
            OSD.GUI.checkAndProcessSymbolPosition(
                hudCenterPosition,
                SYM.AH_CROSSHAIRS[crsHNumber][1]
            );
            OSD.GUI.checkAndProcessSymbolPosition(
                hudCenterPosition + 1,
                SYM.AH_CROSSHAIRS[crsHNumber][2]
            );
        } else {
            ...
            OSD.GUI.checkAndProcessSymbolPosition(
                hudCenterPosition,
                SYM.AH_CROSSHAIRS[crsHNumber]
            );
            OSD.GUI.checkAndProcessSymbolPosition(
                hudCenterPosition + 1,
                SYM.AH_CENTER_LINE_RIGHT
            );
        }
    }
    ...
    if ($('input[name="HORIZON_SIDEBARS"]').prop("checked")) {
        ...
        for (i = -hudheight; i <= hudheight; i++) {
            OSD.GUI.checkAndProcessSymbolPosition(
                hudCenterPosition - hudwidth + i * FONT.constants.SIZES.LINE,
                SYM.AH_DECORATION
            );
            OSD.GUI.checkAndProcessSymbolPosition(
                hudCenterPosition + hudwidth + i * FONT.constants.SIZES.LINE,
    ...
    for (i = 0; i < OSD.data.display_size.total; ) {
        ...
        if (typeof charCode === "object") {
            ...
            if (OSD.data.preview[i][2] !== undefined) {
                ...
                colorStyle =
                    'style="background-color: ' + OSD.data.preview[i][2] + ';"';
            }
        }
        var $img = $(
            '<div class="char"' +
...
OSD.GUI.updateAll = function () {
    ...
    if (OSD.data.layout_count > 1) {
        ...
        for (var ii = 0; ii < OSD.data.layout_count; ii++) {
            var name =
                ii > 0
                    ? chrome.i18n.getMessage("osdLayoutAlternative", [ii])
                    : chrome.i18n.getMessage("osdLayoutDefault");
        ...
        layouts.on("change", function () {
            ...
            OSD.GUI.updateGuidesView(
                $("#videoGuides").find("input").is(":checked")
            );
        ...
        Settings.processHtml(function () {
            ...
            chrome.storage.local.get("osd_font", function (result) {
                if (result.osd_font != undefined) {
                    previous_font_button = $(
                        '.fontbuttons button[data-font-file="' +
                            result.osd_font +
                            '"]'
                    );
                    if (
                        previous_font_button.attr("data-font-file") == undefined
                    )
                        previous_font_button = undefined;
                }
            ...
            $("a.flash_font").click(function () {
                if (!GUI.connect_lock) {
                    ...
                    var uploading = chrome.i18n.getMessage(
                        "uploadingCharacters"
                    );
                    ...
                    var progressCallback = function (done, total, percentage) {
                        ...
                        if (done == total) {
                            progressLabel.text(
                                chrome.i18n.getMessage("uploadedCharacters"),
                                [total]
                            );
                        } else {
                            progressLabel.text(
            ...
            $(document).on(
                "click",
                "span.progressLabel a.save_font",
                function () {
                    //noinspection JSUnresolvedVariable
                                    ...
                                    chrome.fileSystem.isWritableEntry(
                                        ...
                                        function (isWritable) {
                                            if (isWritable) {
                                                ...
                                                console.log(
                                                    "You don't have write permissions for this file, sorry."
                                                );
                                                GUI.log(
            ...
            MSP.send_message(
                MSPCodes.MSP2_CF_SERIAL_CONFIG,
                false,
                false,
                function () {
                    for (
                        var portIndex = 0;
                        portIndex < SERIAL_CONFIG.ports.length;
                        portIndex++
                    ) {
...
function createCustomElements() {
    ...
    for (var i = 0; i < OSD_CUSTOM_ELEMENTS.settings.customElementsCount; i++) {
        ...
        var customElementTable = $("<table>").addClass(
        ...
        for (var ii = 0; ii < 3; ii++) {
            var select = $("<select>")
                .addClass("osdCustomElement-" + i + "-part-" + ii + "-type")
                .data(
                    "valueCellClass",
                    "osdCustomElement-" + i + "-part-" + ii + "-value"
                ).html(`
        ...
        var selectVisibility = $("<select>")
            .addClass("osdCustomElement-" + i + "-visibility-type")
            .data(
                "valueCellClass",
                "osdCustomElement-" + i + "-visibility-value"
            ).html(`
        ...
        customElementRowValue.append(
            $("<td>")
                .addClass("osdCustomElement-" + i + "-visibility-value")
                .append(
                    $("<select>")
                        .addClass("value")
                        .addClass("gv")
        ...
        customElementLabel.append(
            $("<td>")
                ...
                .append(
                    $("<span>").html(
                        chrome.i18n.getMessage("custom_element") + " " + (i + 1)
                    )
                )
        );

        customElementTable
            .append(customElementRowType)
            .append(customElementRowValue)
            .append(customElementLabel);
...
function fillCustomElementsValues() {
    for (var i = 0; i < OSD_CUSTOM_ELEMENTS.settings.customElementsCount; i++) {
        for (var ii = 0; ii < 3; ii++) {
            $(".osdCustomElement-" + i + "-part-" + ii + "-type")
                .val(OSD_CUSTOM_ELEMENTS.items[i].customElementItems[ii].type)
                .trigger("change");
            ...
            switch (OSD_CUSTOM_ELEMENTS.items[i].customElementItems[ii].type) {
                case 1:
                    valueCell
                        .find(".text")
                        .val(OSD_CUSTOM_ELEMENTS.items[i].customElementText)
                        .trigger("change");
                    break;
                case 2:
                    valueCell
                        .find(".ico")
                        .val(
                            OSD_CUSTOM_ELEMENTS.items[i].customElementItems[ii]
                                .value
                        )
                        .trigger("change");
                    break;
        ...
        $(".osdCustomElement-" + i + "-visibility-type")
            .val(OSD_CUSTOM_ELEMENTS.items[i].customElementVisibility.type)
            .trigger("change");
        var valueVisibilityCell = $(
            ".osdCustomElement-" + i + "-visibility-value"
        );
        switch (OSD_CUSTOM_ELEMENTS.items[i].customElementVisibility.type) {
            case 1:
                valueVisibilityCell
                    .find(".gv")
                    .val(
                        OSD_CUSTOM_ELEMENTS.items[i].customElementVisibility
                            .value
                    )
                    .trigger("change");
                break;
...
function customElementsInitCallback() {
    var callback = function () {
        ...
        MSP.promise(
            MSPCodes.MSP2_INAV_SET_CUSTOM_OSD_ELEMENTS,
            customElementGetDataForRow(row)
        );
    };
...
function customElementNormaliseRow(row) {
    for (var i = 0; i < 3; i++) {
        var elementType = $(
            ".osdCustomElement-" + row + "-part-" + i + "-type"
        );
        ...
        switch (parseInt(elementType.val())) {
            case 1:
                valueCell
                    .find(".text")
                    .val(valueCell.find(".text").val().toUpperCase());
                valueCell.find(".text").val(
                    valueCell
                        .find(".text")
                        .val()
                        .replace(/[^A-Z0-9!.\* ]/g, "")
            ...
            case 2:
                valueCell
                    .find(".ico")
                    .val(
                        valueCell.find(".ico").val() > 255
                            ? 255
                            : valueCell.find(".ico").val()
                    );
                valueCell
                    .find(".ico")
                    .val(
                        valueCell.find(".ico").val() < 1
                            ? 1
                            : valueCell.find(".ico").val()
                    );
        }
...
function customElementGetDataForRow(row) {
    ...
    for (var ii = 0; ii < 3; ii++) {
        var elementType = $(
            ".osdCustomElement-" + row + "-part-" + ii + "-type"
        );
        var valueCell = $("." + elementType.data("valueCellClass"));
        var partValue = 0;
        ...
        switch (parseInt(elementType.val())) {
            ...
            case 3:
                partValue = parseInt(
                    valueCell.find(".ico_gv").find(":selected").val()
                );
                break;
            case 4:
            case 5:
            ...
            case 7:
                partValue = parseInt(
                    valueCell.find(".gv").find(":selected").val()
                );
                break;
        }
    ...
    var elementVisibilityType = $(
        ".osdCustomElement-" + row + "-visibility-type"
    );
    var valueVisibilityCell = $(
        "." + elementVisibilityType.data("valueCellClass")
    );
    var visibilityValue = null;
    switch (parseInt(elementVisibilityType.val())) {
        case 1:
            visibilityValue = parseInt(
                valueVisibilityCell.find(".gv").find(":selected").val()
            );
            break;
        case 2:
            visibilityValue = parseInt(
                valueVisibilityCell.find(".lc").find(":selected").val()
            );
            break;
    }
    ...
    for (
        var i = 0;
        i < OSD_CUSTOM_ELEMENTS.settings.customElementTextSize;
        i++
    ) {
        if (i < text.length) {
        ...
        if ($("#osdSwitchInd" + si + "_name").val() != undefined) {
            ...
            if (switchIndText == "") {
                ...
                if ($("#switchIndicators_alignLeft").prop("checked")) {
                    item.preview =
                        switchIndText + FONT.symbol(SYM.SWITCH_INDICATOR_HIGH);
                } else {
                    item.preview =
                        FONT.symbol(SYM.SWITCH_INDICATOR_HIGH) + switchIndText;
                }
            }
        }
    }
...
function updatePilotAndCraftNames() {
    ...
    if (
        $("#craft_name").val() != undefined ||
        $("#pilot_name").val() != undefined
    ) {
            ...
            if (servo == null) {
                ...
                $(this).text(
                    "Servo " +
                        servoIndex +
                        ": " +
                        FC.getServoMixInputName(servoInputIndex)
                );
            }
        }
    });
    ...
    for (let si = 0; si < generalGroup.items.length; si++) {
        if (generalGroup.items[si].name == "PAN_SERVO_CENTRED") {
            ...
            if ($("#osdPanServoIndicatorShowDegrees").prop("checked")) {
                generalGroup.items[si].preview =
                    FONT.symbol(SYM.PAN_SERVO_IS_OFFSET_L) +
                    "120" +
                    FONT.symbol(SYM.DEGREES);
            } else {
                generalGroup.items[si].preview = FONT.symbol(
                    SYM.PAN_SERVO_IS_OFFSET_L
                );
            }
            break;
        }
